...
4. [Core Message Format](#4-hibp-v1-core-message-format)  
...
5. [Extended Payload Format](#5-extended-payload-format-optional)  
6. [Intent Code Registry](#6-hibp-v1-intent-code-registry)  
...
The **Hyperlocal Intent Broadcast Protocol (HIBP)** is an open, lightweight communication protocol for broadcasting short‑range, privacy‑preserving **intent signals** using Bluetooth Low Energy (BLE), Wi‑Fi, or other local transports.

HIBP enables devices to share hyperlocal human‑intent metadata—such as, but not limited to, items for sale, lost items, free items, services, and local events—without accounts, logins, or centralized platforms.
...
Python gateway (receiver + normalizer)

Android broadcaster (BLE advertiser)
...
HIBP‑v1 is published as an open standard under a permissive license (MIT or similar).
